...
function SpeakStone:CreateWindow()
    ...
    local function RefreshOptionStates()
        local master = SpeakStone_ForeverMainDB.autoPlayEnabled
    ...
    local function makeCheckButton(info)
        ...
        checkButton:SetChecked(SpeakStone_ForeverMainDB[info.option])

        checkButton:SetScript("OnClick", function(self)
            SpeakStone_ForeverMainDB[info.option] = self:GetChecked()
            if info.onChange then info.onChange(self:GetChecked()) end
            RefreshOptionStates()
        end)
        checkButton:SetScript("OnShow", function(self)
            self:SetChecked(SpeakStone_ForeverMainDB[info.option])
        end)
    ...
    for index, section in ipairs(SETTINGS_SECTIONS) do
        ...
        if section.title == "Playback" then
            ...
            slider:SetScript("OnValueChanged", function(self, value)
                ...
                SpeakStone_ForeverMainDB.autoPlayDelay = value
                Describe(value)
            end)
            slider:SetScript("OnShow", function(self)
                local value = tonumber(SpeakStone_ForeverMainDB.autoPlayDelay) or 0.5
                self:SetValue(value)
                Describe(value)
            end)
    ...
    local function RefreshDashboard()
        ...
        elseif SpeakStone_ForeverMainDB.harvestEnabled then
