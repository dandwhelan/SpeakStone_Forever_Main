...
local function AutoPlayAllowed(kind)
    if not SpeakStone_ForeverMainDB or not SpeakStone_ForeverMainDB.autoPlayEnabled then
        return false
    end
    return SpeakStone_ForeverMainDB[kind] ~= false
end
...
local function DebugPrint(...)
    if SpeakStone_ForeverMainDB and SpeakStone_ForeverMainDB.showDebugMessages then
        print(...)
    end
end
...
local function InitializeAddonDB()
    ...
    for option, default in pairs(defaultSettings) do
        if SpeakStone_ForeverMainDB[option] == nil then
            SpeakStone_ForeverMainDB[option] = default
        end
    end
    ...
    if not SpeakStone_ForeverMainDB.migratedAutoPlayDelayV2 then
        SpeakStone_ForeverMainDB.migratedAutoPlayDelayV2 = true
        if SpeakStone_ForeverMainDB.autoPlayDelay == 2 then
            SpeakStone_ForeverMainDB.autoPlayDelay = 0.5
        end
    end
    ...
    if not SpeakStone_ForeverMainDB.migratedAutoPlayDelayV3 then
        SpeakStone_ForeverMainDB.migratedAutoPlayDelayV3 = true
        if SpeakStone_ForeverMainDB.autoPlayDelay == 0.5 then
            SpeakStone_ForeverMainDB.autoPlayDelay = 1.5
        end
    end

    SpeakStone_ForeverMainDB.IsPaused = false
    SpeakStone_ForeverMainDB.IsSoundPaused = false
...
local function RestoreStrandedDialogVolume()
    ...
    if not SpeakStone_ForeverMainDB then return end
    local saved = SpeakStone_ForeverMainDB.savedDialogVolume
    SpeakStone_ForeverMainDB.savedDialogVolume = nil
...
local questReaderLauncher = LDB:NewDataObject("SpeakStone_Main", {
    ...
    OnClick = function(_, button)
        ...
        elseif button == "MiddleButton" then
            SpeakStone_ForeverMainDB.showDebugMessages = not SpeakStone_ForeverMainDB.showDebugMessages
            print("SpeakStone Debug Messages: " .. (SpeakStone_ForeverMainDB.showDebugMessages and "Enabled" or "Disabled"))
        end
    end,
    OnTooltipShow = function(tooltip)
        tooltip:AddLine("SpeakStone")
        tooltip:AddLine("|cffffffffLeft-Click:|r Open Settings", 1, 1, 1)
        tooltip:AddLine("|cffffffffMiddle-Click:|r Toggle Debug Messages (" .. ((SpeakStone_ForeverMainDB and SpeakStone_ForeverMainDB.showDebugMessages) and "|cff00ff00On|r" or "|cffff0000Off|r") .. ")", 1, 1, 1)
...
local function UpdateMinimapButtonVisibility()
    ...
    SpeakStone_ForeverMainDB.minimapButton = SpeakStone_ForeverMainDB.minimapButton or {}
    SpeakStone_ForeverMainDB.minimapButton.hide = not SpeakStone_ForeverMainDB.showMinimapButton
    if SpeakStone_ForeverMainDB.showMinimapButton then
        icon:Show("SpeakStone_Main")
        C_Timer.After(0.1, function()
            local minimapButton = icon:GetMinimapButton("SpeakStone_Main")
            if minimapButton and SpeakStone_ForeverMainDB.minimapIconPosition.point then
                minimapButton:ClearAllPoints()
                minimapButton:SetPoint(
                    SpeakStone_ForeverMainDB.minimapIconPosition.point,
                    Minimap,
                    SpeakStone_ForeverMainDB.minimapIconPosition.relPoint,
                    SpeakStone_ForeverMainDB.minimapIconPosition.x,
                    SpeakStone_ForeverMainDB.minimapIconPosition.y
                )
            end
        end)
    else
    ...
    if event == "ADDON_LOADED" and loadedAddonName == addonName then
        ...
        if questReaderLauncher then
            icon:Register("SpeakStone_Main", questReaderLauncher, {
                hide = not SpeakStone_ForeverMainDB.showMinimapButton,
                position = SpeakStone_ForeverMainDB.minimapIconPosition
            })
            C_Timer.After(0.5, function()
                ...
                if minimapButton then
                    minimapButton:HookScript("OnDragStop", function()
                        ...
                        SpeakStone_ForeverMainDB.minimapIconPosition = {
                            point = point,
                            relPoint = relPoint,
                            x = x,
                            y = y
...
local function UpdateQuestButtonVisibility()
    local show = SpeakStone_ForeverMainDB.showQuestButton
...
SlashCmdList["QRTOGGLE"] = function()
    SpeakStone_ForeverMainDB.showMinimapButton = not SpeakStone_ForeverMainDB.showMinimapButton
    UpdateMinimapButtonVisibility()
    print("SpeakStone minimap button: " .. (SpeakStone_ForeverMainDB.showMinimapButton and "|cff00ff00shown|r" or "|cffff0000hidden|r"))
end
...
local function DoPlaySound(soundData)
    ...
    if SpeakStone_ForeverMainDB.muteGossip then
        MuteDialogChannel()
        audioChannel = "Master"
    end
...
local function NPCVoiceGap()
    return tonumber(SpeakStone_ForeverMainDB.autoPlayDelay) or 1.5
end

local function YieldingToNPCVoice()
    return SpeakStone_ForeverMainDB.yieldToNPCVoice ~= false and not SpeakStone_ForeverMainDB.muteGossip
end
...
function PlayQuestAudio(textType, skipDelay)
    ...
    if questID and textType ~= "" then
        ...
        if SpeakStone_ForeverMainDB.autoPlayEnabled and not skipDelay and not SpeakStone_ForeverMainDB.muteGossip then
            ScheduleSound(soundData, tonumber(SpeakStone_ForeverMainDB.autoPlayDelay) or 1.5)
        else
            DoPlaySound(soundData)
        end
    end
...
questEventFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    if textType ~= "" and AutoPlayAllowed("autoPlayQuests") then
        if QuestMapFrame and QuestMapFrame:IsVisible() and not (QuestFrame and QuestFrame:IsVisible()) then
            if not SpeakStone_ForeverMainDB.autoPlayInQuestMap then
...
local function NoteNPCVoice(seconds, interruptPlaying)
    if not SpeakStone_ForeverMainDB or not YieldingToNPCVoice() then
        return
    end
...
npcVoiceFrame:SetScript("OnEvent", function(_, event, ...)
    ...
    elseif event == "TALKINGHEAD_CLOSE" then
        ...
        local current = GetCurrentSound()
        if current and not current.isPlaying and current.nextSoundTimer and SpeakStone_ForeverMainDB then
            ScheduleSound(current, 0)
        end
    else
        local text, sender = ...
...
SlashCmdList["QUESTREADERAUTO"] = function(msg)
    if msg == "on" then
        SpeakStone_ForeverMainDB.autoPlayEnabled = true
    elseif msg == "off" then
        SpeakStone_ForeverMainDB.autoPlayEnabled = false
    else
        SpeakStone_ForeverMainDB.autoPlayEnabled = not SpeakStone_ForeverMainDB.autoPlayEnabled
    end
    print("SpeakStone Auto-Play: " .. (SpeakStone_ForeverMainDB.autoPlayEnabled and "Enabled" or "Disabled"))
end
...
SlashCmdList["QUESTREADERDEBUG"] = function(msg)
    if msg == "on" then
        SpeakStone_ForeverMainDB.showDebugMessages = true
    elseif msg == "off" then
        SpeakStone_ForeverMainDB.showDebugMessages = false
    else
        SpeakStone_ForeverMainDB.showDebugMessages = not SpeakStone_ForeverMainDB.showDebugMessages
    end
    print("SpeakStone Debug Messages: " .. (SpeakStone_ForeverMainDB.showDebugMessages and "Enabled" or "Disabled"))
...
if not (C_AddOns and C_AddOns.IsAddOnLoaded and C_AddOns.IsAddOnLoaded("SpeakStoneHarvester")) then
    ...
    SlashCmdList["QUESTREADERHARVEST"] = function(msg)
        ...
        elseif msg == "on" or msg == "off" then
            SpeakStone_ForeverMainDB.harvestEnabled = (msg == "on")
        ...
        print("SpeakStone capture: " .. (SpeakStone_ForeverMainDB.harvestEnabled and "|cff00ff00on|r" or "|cffff0000off|r"))
...
function MuteDialogChannel()
    ...
    if not originalDialogVolume then
        ...
        if SpeakStone_ForeverMainDB then
            SpeakStone_ForeverMainDB.savedDialogVolume = originalDialogVolume
        end
    end
